...
// keymap_langmap.go normalizes keystrokes that arrive under a CJK input source.
//
...
// which is what the same physical chord would have produced in English — so
// `ㅆ` reaches an uppercase-only binding like `M` would.
...
// into the composer, the search box, the palette, and the pickers verbatim.
// Exported so the sibling actions/ and admin/ TUIs share one mapping.
